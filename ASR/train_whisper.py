"""Whisper ASR fine-tuning for SLURP-TN.
...
class WhisperASR(sb.Brain):
    """SpeechBrain Brain class for Whisper ASR fine-tuning."""
    ...
    def compute_objectives(self, predictions, batch, stage):
        """Compute loss and store predictions for ASR evaluation."""
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        stage_stats.update(compute_asr_metrics(self.prediction_store))
        ...
        if stage == sb.Stage.VALID:
            ...
            if if_main_process():
                ...
                metrics = compute_metrics_from_jsonl(
                    predictions_file=predictions_file,
                    task=getattr(self.hparams, "task", "asr"),
                )
...
def dataio_prepare(hparams, tokenizer):
    ...
    target_column = hparams.get("target_column", "tun_transcription")
...
def patch_searcher(searcher, default_length=448):
    """Patch the searcher if max_attn_tokens is missing."""
...
if __name__ == "__main__":
    ...
    whisper_asr_brain.tokenizer = whisper_asr_brain.modules["whisper"].tokenizer

    logger.info("Preparing datasets...")

    train_data, valid_data, test_data = dataio_prepare(
        hparams,
        whisper_asr_brain.tokenizer,
    )

    if hasattr(whisper_asr_brain.hparams, "valid_search"):
        patch_searcher(whisper_asr_brain.hparams.valid_search)

    if hasattr(whisper_asr_brain.hparams, "test_search"):
        patch_searcher(whisper_asr_brain.hparams.test_search)

    logger.info("Starting training...")

    whisper_asr_brain.fit(
        whisper_asr_brain.hparams.epoch_counter,
        train_data,
        valid_data,
    ...
    if if_main_process():
        Path(whisper_asr_brain.hparams.dev_scores_dir).mkdir(
            parents=True,
            exist_ok=True,
        )
        Path(whisper_asr_brain.hparams.test_scores_dir).mkdir(
            parents=True,
            exist_ok=True,
        )
    ...
    whisper_asr_brain.scores_dir = whisper_asr_brain.hparams.dev_scores_dir
    whisper_asr_brain.evaluate(
        valid_data,
    ...
    whisper_asr_brain.scores_dir = whisper_asr_brain.hparams.test_scores_dir
    whisper_asr_brain.evaluate(
        test_data,
        test_loader_kwargs=hparams["test_loader_kwargs"],
        min_key="WER",
